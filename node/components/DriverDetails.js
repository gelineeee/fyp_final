...
const style = {
  ...
  buttonSize: `px-6 py-2 flex flex-col`,
  button1: `py-2 px-6 my-2 bg-green-600 hover:bg-green-700 focus:ring-green-500 focus:ring-offset-green-200 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-lg`,
  button2: `py-2 px-6 my-2 bg-red-500 hover:bg-red-700 focus:ring-red-500 focus:ring-offset-indigo-200 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-lg`,
  button3: `py-2 px-6 my-12 bg-green-600 hover:bg-green-700 focus:ring-green-500 focus:ring-offset-green-200 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-lg`,
};
...
const DriverDetails = () => {
    ...
    (async () => {
      try {
        const response = await fetch(
          `/api/db/getUserInfo?walletAddress=${driverRide[0].passenger._ref}`
        );

        const data = await response.json();
        setPhone(data.data.contactNumber);
        setRiderName(data.data.name);
  ...
  const startRide = async () => {
    try {
      ...
      const hitchInstance = Hitch(driverRide[0].contractAddress);
      await hitchInstance.methods.startHitch().send({
        from: accounts[0],
      });
  ...
  const dropoff = async () => {
    try {
      ...
      const accounts = await web3.eth.getAccounts();
      const hitchInstance = Hitch(driverRide[0].contractAddress);
      await hitchInstance.methods.endHitch().send({
        from: accounts[0],
      });
  ...
  const cancelRide = async () => {
    try {
      ...
      const accounts = await web3.eth.getAccounts();
      const hitchInstance = Hitch(driverRide[0].contractAddress);
      await hitchInstance.methods.driverCancel().send({
        from: accounts[0],
      });
    ...
    <div className={style.wrapper}>
      ...
      <div className={style.details}>
        ...
        <div class="absolute -right-3 flex flex-row">
          <div class="-mr-2">{driverRide[0].price}</div>
      ...
      <div class="flex flex-row mb-2">
        ...
        <div className={style.coldetails}>
          <div>{driverRide[0].dropoff}</div>
          <div class="text-xs font-normal">Destination</div>
        </div>
      </div>

      {driverRide[0].rideStatus == "Ongoing" ? (
